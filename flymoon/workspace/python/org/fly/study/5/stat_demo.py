...
def calc_statistics(x):
    ...
    kurtosis = (m4 - 4 * mu * m3 + 6 * mu * mu * m2 - 4 * mu ** 3 * mu + mu ** 4) / sigma
    print('mu=', mu, ':m2=', m2, ':m3=', m3, ':m4=', m4)
...
if __name__ == '__main__':
    ...
    c=np.random.randn(10000,2)
    mu,sigma,skew,kurtosis=calc_statistics(c)
    print('函数库计算均值、标准差、偏度、峰度：', mu, sigma, skew, kurtosis)

    #二维图像
    N=30
    density,edges=np.histogramdd(c,bins=[N,N])
    print('样本总数：',np.sum(density))
    print("density=",density,"edges=",edges)
    density/=density.max()
    x=y=np.arange(N)
    print('x=',x)
    print('y=',y)
